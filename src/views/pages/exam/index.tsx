...
export const ExamPage = () => {
  ...
  return (
    <>
      {opened ? <Modal opened={opened} onClose={() => { }} title="Password" overlayProps={{ blur: 4 }} closeOnClickOutside={false} withCloseButton={false}>
        <PasswordInput placeholder='Password' value={password} onChange={(e) => setPassword(e.target.value)} />
        <Button mt={10} style={{ display: 'flex', marginLeft: 'auto' }} size='xs' disabled={password.length < 1} onClick={handleSubmitPassword}>Enter</Button>
      </Modal> : <Flex p="md" direction="column" align="center" bg="#d5dbd5">
        ...
        <FormExam exam={{ _id: testInfo?._id as string, questions: testInfo.questions }} expired={expired} />
      </Flex>}
    </>
  );
};
